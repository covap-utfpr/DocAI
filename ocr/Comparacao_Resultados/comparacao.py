...
def comparar_consumindo(base, cupom):                       # Comparação entre o cupom travado de referencia e os demais
    ...
    for item in base:                                       # Para cada item no cupom de referencia 
        if cupom_counter[item] > 0:                         # Se o item é encontrado no cupom             
            ...
            con_encontrado[item] -= 1                       # consome uma linha do contador 
...
def main():                                                                     # Função principal
    ...
    base_nome = base_path.replace(".txt", "").split("/")[-1]                    # Nome da base sem extensão e caminho
    base = extrair_ocr(base_path)                                               # Extrai tokens da base (função)
    ...
    for path, nome in zip(cupons_paths, nomes_cupons):                  # Para cada caminho e nome de cupom em zip (lista paralela)
        cupom = extrair_ocr(path)                                       # Extrai tokens do cupom (função)
    ...
    plt.figure(figsize=(len(itens_base)*0.4, 6))                    # Figura maior para muitos itens, ajustando largura baseado em número de itens, altura fixa 
    ...
    plt.savefig("grafico_acertos_erros_longo.png")                  # Salva o gráfico como imagm
