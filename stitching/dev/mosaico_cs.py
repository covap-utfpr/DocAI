...
Original file is located at
    https://colab.research.google.com/drive/1A73zCyPHn_cHgG8OzgAR5MEvPzo4ssDn
    ...
    match op:
        ...
        case 3:
            ...
            if (alvo - saltoFixo)  < 0:
                ...
                return alvo - saltoFixo

        case 4:

            if (alvo + saltoFixo ) <= (len(frames) - 1):
                return alvo + saltoFixo
...
while(1):
    matches, base_image_kp, sec_image_kp = find_matches(frames[idx_base], frames[idx_sec], threshold=0.7, debug_draw=True, min_matches=100)
    if(matches != None):
        break
    else:
        idx_sec+=1
...
while(1):
    ...
    del frames[alvo:]
    alvo = math.ceil(len(frames) / 2)

    if (len(frames)) == alvo:  # Alvo == Pivo
        print("Finalizando")
        break